...
function Login({ setIsLogin }) {
    ...
    const registerSubmit = async e => {
        e.preventDefault();
        try {
            const res = await axios.post('https://notes-backend-5zui.onrender.com/users/register', {
    ...
    const loginSubmit = async e => {
        e.preventDefault();
        try {
            const res = await axios.post('https://notes-backend-5zui.onrender.com/users/login', {
                email: user.email,
                password: user.password
            });
            setUser({ username: '', email: '', password: '' }); // Clear form fields
